...
def _build_embedder() -> EmbeddingClient:
    ...
    # 测试环境或无 key：用确定性 Fake，方便无凭证试导入
    if settings.testing or not settings.openai_api_key:
        print("提示：TESTING=true 或未配置 OPENAI_API_KEY，使用 FakeEmbeddingClient（伪向量）")
...
def main() -> None:
    ...
    report = asyncio.run(import_knowledge_csv(
        args.csv_path, embedder=_build_embedder(), brand_id_default=args.brand,
    ))
    print(
