...
async def test_human_handoff_repair_downgrade_and_reupgrade():
    ...
    database_name = f"social_reply_handoff_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        async with engine.begin() as connection:
            ...
            await connection.execute(
                text(
                    ...
                    "'user-' || n FROM generate_series(1, 8) AS n"
                )
            )
            await connection.execute(
                text(
                    ...
                    "'handoff-' || n, 'dm', 0 FROM generate_series(1, 8) AS n"
                )
            )
            await connection.execute(
                text(
                    ...
                    "('30000000-0000-0000-0000-000000000008', 'BOT_DRAFT_ONLY', 1, NULL, 'MANUAL')"
                )
            )
            await connection.execute(
                text(
        ...
        assert versions == [2, 2, 2, 1, 1, 2, 2, 2]
        await engine.dispose()
    finally:
        async with admin_engine.connect() as connection:
            await connection.execute(
