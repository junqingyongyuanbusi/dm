...
def test_email_documentation_and_migration_head_contract() -> None:
    ...
    assert _migration_heads(root / "migrations/versions") == {"c3e7a9f1b204"}

    production_migration = (root / "docs/production-migration.md").read_text()
    docs_readme = (root / "docs/README.md").read_text()
    root_readme = (root / "README.md").read_text()
    assert re.search(
        r"current Alembic graph has one head: `c3e7a9f1b204`",
        production_migration,
    )
    assert re.search(
        r"Alembic graph has one current head:\s*`c3e7a9f1b204`",
        docs_readme,
    )
    assert re.search(
        r"current revision 等于唯一 head `c3e7a9f1b204`",
        root_readme,
    )
...
def test_multilingual_runtime_forces_english_scope_in_code_not_settings() -> None:
    ...
    assert settings.multilingual_knowledge_reply_enabled is True
    assert settings.english_knowledge_only_enabled is False
