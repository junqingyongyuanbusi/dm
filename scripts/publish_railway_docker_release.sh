...
usage() {
  cat <<'EOF'
Migration-aware fallback: publish the current clean dev commit to Docker Hub and deploy it only when Railway services have been placed on the reviewed image-source path.
...
script_path="$repo_root/scripts/publish_railway_docker_release.sh"
