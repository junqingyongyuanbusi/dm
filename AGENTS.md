...
只有用户在当前请求中明确要求部署或让代码在 Railway 生效时，才执行“提交到 `dev` → 推送 `origin/dev` → 等待该 SHA 的 CI 全绿 → 构建并发布 GHCR immutable SHA 镜像 → 提升同一 digest 为 `latest` → 通过发布脚本更新 Railway 三角色 → 验证健康与 digest”的完整链路。`railway restart` 只重启现有容器，不会携带未提交、未构建或未发布的代码，不得作为代码发布的替代。
...
6. 如果 GitHub、GHCR 或 Railway 凭证不可用，准确报告对应的 commit、push、CI 或 release 阻塞状态，不得把未完成阶段说成已完成。

### GHCR 发布契约

- 每次 `dev` push 在 CI 全绿后自动发布 immutable SHA 镜像；这一步不移动 production `latest`，也不代表 Railway 已发布。
...
- `api`、`worker`、`scheduler` 的 source image 必须保持 `ghcr.io/junqingyongyuanbusi/reply-core:latest`。
...
- 最终验证必须证明 API、Worker、Scheduler 运行完全相同且等于 GHCR `latest` 的 digest，并确认 API、Worker、Scheduler、PostgreSQL、Redis 区域一致。
