...
| Feishu duplicate event | The same authenticated callback `header.event_id` is delivered more than once, including while the feature is disabled | The account-scoped partial unique RawEvent index admits one ingress row; replays return 200 without a second RawEvent or dispatch. |
...
| Local inbox manual send | Operator replies from the built-in inbox to an explicit inbound target | A tenant-scoped claim/create and `HUMAN_ACTIVE` transition commit with a `MANUAL_REPLY/ADMIN_HUMAN` Outbox; successful delivery writes an outbound `Message.source_outbox_id` without requiring a ReplyDecision. Delivery uses the account's explicit direct provider target. |
...
- Local human work, explicit reply targets and manual-send provenance remain durable and tenant-scoped. Account automation policy is the restore target, work items affect one conversation only, and ignored inbound work is never retroactively decided; all supported operations use direct provider destinations.
