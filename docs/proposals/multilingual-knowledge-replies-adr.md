...
实施记录见 `docs/superpowers/plans/2026-08-22-phase2-plan4-any-language-reply.md`。本节只记录
与本 ADR 判断相关的四点，其余细节不重复。

**「固定 10 语言 allowlist」这一行的前提已经不成立。** `MULTILINGUAL_SUPPORTED_LANGUAGES` 在代码里
零引用，是上一轮迁移的残留，已从 Railway 三个服务删除。真正限制语言覆盖的是检测层的分支覆盖、
guard 的脚本表与时间单位词表，与 allowlist 无关——这批死配置还误导过一次排查（一度以为线上跑的是
0.5/0.001，实际是 0.8/0.08）。

**语言解析改为三级级联，但检测层本身未替换。** 确定性检测判不出或落在近亲语言对上时改由 LLM 判定，
`domain/reply/language.py` 保持纯同步确定性不变。这没有解决本 ADR 对 Lingua「170 MB wheel + 400 行
启发式」的成本质疑，只是把它的失效面兜住了；替换评测仍应照做。新增的实测证据：Lingua 在天城文上
会给出**高置信度的错误答案**（`नमस्ते` → mr，置信度 0.624，反而高于正确判定的俄语 0.383），
说明置信度不能作为其可靠性的代理指标。

**「第二次同模型 grounding 应从热路径删除」这条建议现在有了反向约束。** 时间单位比对在 7 种语言上
（de/it/vi/tr/nl/pl/sv）识别不出单位，已改为降级放行并交 grounding verifier 兜底。删除或抽样化
grounding 之前，必须先补回这部分确定性校验，否则这些语言的事实防线会归零。已用对抗样本验证
verifier 能拦下单位与数值篡改（35 次调用全部符合预期）；同时观察到约 3% 的调用因中转网络异常
返回 `False`，即 **verifier 的可用性直接决定自动回复率上限**。

**「lexical/RRF 当前付出成本但不使用其授权结果」有了具体量化。** `question_tsv` 用 `simple` 分词器
索引英文 question，非英语查询切出的词元与英文词元永远不相交——**词法臂对非英语查询召回恒为零**，
混合检索退化成纯向量。这使 `query_translation` 成为非英语链路上唯一能恢复词法召回的路径，
而它的失败是静默的。删或留之前应把这一点计入。
