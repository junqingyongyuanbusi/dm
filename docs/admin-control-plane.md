...
- draft-first knowledge management at `/admin/content/knowledge`, with `/admin/knowledge` retained for compatibility: manual and CSV-created rows cannot be retrieved until an explicit tenant-scoped publish action. Administrators can bulk-publish ordinary drafts for one explicit Tenant; the server excludes both classified official-contact rows and unclassified replies that deterministically look like contact information, while updating eligible rows atomically and writing one `PUBLISH_KNOWLEDGE` audit per row. Single-row publication and withdrawal remain row-locked, idempotent, and audited. Official-contact classification is explicit, draft-only, tenant-scoped, row-locked, and audited as `SET_KNOWLEDGE_OFFICIAL_CONTACT`; only a published, verbatim-selected classified template can pass the deterministic contact PII exception;
