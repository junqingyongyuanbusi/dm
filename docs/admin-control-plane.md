...
- editable Tenant + Brand business instructions at `/admin/content/reply-prompt`, with `/admin/content/brand-voice` and `/admin/prompt` retained as compatibility routes: the page displays the active text, accepts direct free-text edits, appends an immutable version, atomically advances the active pointer, and audits the change as `SET_REPLY_BUSINESS_PROMPT`. Rollback copies historical content into another immutable active version. The editable text is supplied only to primary reply generation; code-owned identity, output schema, RAG boundaries, language checks, safety guards and delivery authorization remain immutable. Trial redacts customer PII and never persists a decision or Outbox;
- draft-first knowledge management at `/admin/content/knowledge`, with `/admin/knowledge` retained for compatibility: new rows store automatic language detection separately from authoritative English confirmation. Operators can inspect full question/reply text, confirm an individual English or justified unknown draft, or bulk-confirm detector-English drafts from one immutable import batch; mixed/non-English rows require a reviewed English replacement. In English-only mode publication requires `source_language=en` and `language_verified=true`. Bulk publication excludes official-contact rows, contact-like replies, unconfirmed rows and conflicting normalized questions, and writes one audit per published row. Published-corpus mutation is blocked while multilingual shadow or live mode pins a calibrated corpus. Official-contact classification remains explicit, draft-only, tenant-scoped and audited; only an approved verbatim template can pass the deterministic contact PII exception;
