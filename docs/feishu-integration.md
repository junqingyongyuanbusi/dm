...
先将支持 Feishu 的镜像以 `FEISHU_ENABLED=false` 部署到三个角色，确认数据库唯一 head 为 `f3b8c1d4e726` 且旧容器全部退出；再协调重启三个角色并同时设为 `true`。人工通知保持独立的 `FEISHU_HANDOFF_NOTIFICATIONS_ENABLED=false`，直到客服群、operator allowlist、Card Action Callback 和测试卡片全部验证。不要在 API 已开启而 Worker 或 Scheduler 仍关闭/仍运行旧镜像时接入账号。
